...
public class UsuarioDAO {
	...
	public void save(Usuario usuario) {
		...
		try {
			sql = usuario.getIdUsuario() == 0?
					"insert into usuario (senha, email, CPF, nome, localização) values (?, ?, ?, ?, ?)"
					:
					"UPDATE usuario SET senha = ?, email = ?, CPF = ?, nome = ?, localização = ? WHERE idUsuario = ?"
						;
